...
# this will set "foo" setting value
...
class KodifunLogin(xbmcgui.WindowXMLDialog):
    ...
    def __init__(self, strXMLname, strFallbackPath, defaultSkin='Default',
		...
		self.username = ''
		self.password = ''
		self.players = int(__settings__.getSetting("32400"))
		self.counter = 0
		...
		xbmc.log("SPY on __init__ " +str(self.players) + " " + strXMLname)
    ...
    def onInit(self):
		 xbmc.log("SPY onInit(self) " +str(xbmcgui.getCurrentWindowId()) + " " + self.SkinXML)
		 if(self.SkinXML == "splash-players.xml"):
			...
			xbmc.log("SPY onInit2(self) " +str(xbmcgui.getCurrentWindowId()) + " " + self.SkinXML)
    ...
    def onClick(self, controlID):
        ...
        if CTRL_ID_CANCEL == controlID:
            self.close()
             ...
             if (len(self.username)>4) & (len(self.password)>4):
                 xbmc.log("KodiFun Lenght is larger")
             else:
                  xbmc.log("KodiFun Lenght is to short")
             xbmc.log("KodiFun " + self.username)
             # self.close()
             ...
             xbmc.log("KodiFun " + self.password)
        elif controlID == 32037:
             if(int(self.counter)  <= int(self.players - 2)):
                 self.counter = self.counter+1
                 kodifunattrib.setPlayerData(self)
                 xbmc.log("Info for next player=" + str(self.counter) + "/" + str(self.players))
             else:
                 xbmc.log("Info Complete close screen=" + str(self.counter) + "/" + str(self.players))
                 self.counter = 0
                 self.close()
             ...
             xbmc.log("ROLL DICE dice 1= " + str(self.dice1) + " dice 2 = "+ str(self.dice2))
		...
		#set player gender to male
             ...
             if(self.SexChoiceMale == 0):
                 ...
                 self.getControl(32063).setSelected(1)
             xbmc.log("KodiFun SexChoiceMale " + str(self.SexChoiceMale))
		...
		# Limit interactions to male
             ...
             xbmc.log("KodiFun Limit interactions to male = " + str(self.getControl(32065).isSelected()))
        ...
        xbmc.log("KodiFun " + str(controlID))

    def onFocus(self, controlID):
         # self.getControl(32204).setEnabled(0)
        WINDOW = xbmcgui.Window(xbmcgui.getCurrentWindowId())
        xbmc.log("SPY Window " +str(xbmcgui.getCurrentWindowId()))
        self.playmates = WINDOW.getProperty("playmates")
        if WINDOW.getProperty("playmates") != "":
            # try:
             self.players = WINDOW.getProperty("playmates")
        xbmc.log("SPY retrieved property" +str(self.playmates))
    pass
    """
